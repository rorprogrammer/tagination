...
Tooltip.prototype = {
	initialize: function(el, options) {
		...
		/*
			image checking
		*/
		if(this.content.substr(0,4) == "<img"){
			start = this.content.indexOf('src=')+5;
			end	= this.content.indexOf("'",start);
			if(end == -1){
				end	= this.content.indexOf('"',start);
			}
			end = end - start;
			src = this.content.substr(start,end);
			this.content = Builder.node(	'img',{
													src:src 
													}
										);
		}
	...
	setOptions: function(options) {
		this.options = {
			...
			maxWidth: 410,	// Default tooltip width
